...
use Chatman\Support\InputWithPatterns;
use Exception;

class BotWithPatterns
{
    ...
    function __construct(string $jsonFile, bool $ignoreMLForSmallSentences = true)
    {
    ...
    private function NL(): void
    {
        ...
        foreach ($this->data['intents'] as $intent) {

            foreach ($intent['patterns'] as $pattern) {

                $tokens = $this->tokenizer->tokenize($pattern);
                $this->words = array_merge($this->words, $tokens);
                $this->X[] = strtolower($pattern);
                $this->y[] = strtolower($intent['tag']);
            }

            if (!in_array($intent['tag'], $this->classes)) {
                $this->classes[] = $intent['tag'];
            }
        }
    ...
    public function getResponse(string $text): array
    {
        ...
        if (!empty($this->botFinalResponse)) {
            ...
            return [
                "tag" => "_final_",
                "resp" => $this->botFinalResponse
            ];
        }

        $data = new InputWithPatterns(
            $this->model,
            $this->data,
            $this->tokenizer,
            $this->words,
            $this->ignoreMLForSmallSentences,
        ...
        if (empty($response['resp'])) {
            ...
            return [
                "tag" => "_default_",
                "resp" => $this->defaultMsg
            ];
        } else {
            $this->setLastBotResp($response['resp']);
            return $response;
        }
    }
    ...
    //improving json dataset
    public function improveDataset(string $tag, string $userText, string $jsonPath, bool $jsonPretty = false): bool
    {
        ...
        foreach ($data['intents'] as &$intent) {
            ...
            if ($intent['tag'] === $tag) {
                ...
                if (!in_array($userText, $intent['patterns'])) {
                    $intent['patterns'][] = $userText;
                    $improved  = true;
                    break;
                }
